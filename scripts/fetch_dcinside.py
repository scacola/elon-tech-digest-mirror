...
Schema follows the dcinside-scout agent: title, recommend, permalink, body_excerpt, tags.
"""
...
ROW_RE = re.compile(
    r'<tr[^>]+class="ub-content[^"]*"[^>]*?data-no="(?P<no>\d+)"[^>]*>(?P<body>.*?)</tr>',
    re.DOTALL,
)
TITLE_RE = re.compile(
    r'<a[^>]+href="/mgallery/board/view/\?id=' + GALL_ID + r'&amp;no=\d+[^"]*"[^>]*>(?P<t>.*?)</a>',
    re.DOTALL,
)
RECOM_RE = re.compile(r'class="gall_recommend"[^>]*>\s*(\d+)')
COMMENT_RE = re.compile(r'class="reply_num"[^>]*>\[?(\d+)')
AUTHOR_RE = re.compile(r'class="(?:gall_writer|ip)[^"]*"[^>]*data-nick="([^"]+)"')
DATE_RE = re.compile(r'class="gall_date"[^>]*title="([^"]+)"')
BODY_RE = re.compile(r'<div[^>]+class="write_div"[^>]*>(?P<b>.*?)</div>\s*(?:<div|</section)', re.DOTALL)
...
def http_get(url: str, retries: int = 3) -> str | None:
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f"  attempt {attempt}/{retries} fail: {e} for {url}", file=sys.stderr)
            if attempt < retries:
...
def parse_list(html: str) -> list[dict]:
    ...
    for m in ROW_RE.finditer(html):
        ...
        recom = int(recom_m.group(1)) if recom_m else 0
        comm_m = COMMENT_RE.search(body)
        comm = int(comm_m.group(1)) if comm_m else 0
